...
@SuppressLint("SimpleDateFormat") public class GetDate {
	...
	public String GetDayDiffer(String now,String another){
		...
		try{
		  ...
		  long l=Date1.getTime()-Date2.getTime();l=Math.abs(l);
	...
	public String GetTimeFromNow(String paramString)
	  {
		...
		try{
		  ...
		  long sec=((l/1000)-day*24*60-hour*60-min*60);
		    if(day>30)	result=day+"天前";
		    else if(day<30&&day>=1)   result=day+"天前";
		    else if(day<1&&hour>=1) result=hour+"小时前";
		    else if(hour<1&&min>=1) result=min+"分钟前";
		    else result=sec+"秒前";
		} catch (ParseException e) {}
	    return result;
	  }
